  ...
  return (
    <div className="content-card deck-designer-card">
      <div className="section-heading inline-heading">
        ...
        <div className="tag-row"><span className="tag">Snap: 6 in</span><span className="tag">Points: {points.length}</span><span className="tag">Mode: {mode}</span></div>
      </div>
      ...
      <div className="deck-designer-grid">
        <div className="deck-canvas-wrap">
          ...
          <svg ref={svgRef} viewBox={`0 0 ${VIEW_SIZE} ${VIEW_SIZE}`} className="deck-canvas" onPointerMove={handlePointerMove} onPointerUp={() => { setDraggingIndex(null); setDraggingBeamIndex(null); setDraggingStair(false); }}>
            ...
            {points.map((point, index) => { const svgPoint = toSvgPoint(point); return <g key={`${point.x}-${point.y}-${index}`}><circle cx={svgPoint.x} cy={svgPoint.y} r={selectedIndex === index ? 9 : 7} className={selectedIndex === index ? 'deck-point active' : 'deck-point'} onPointerDown={(event) => onPointPointerDown(event, index)} onClick={() => setSelectedIndex(index)} /><text x={svgPoint.x + 10} y={svgPoint.y - 10} className="deck-point-label">P{index + 1}</text></g>; })}
