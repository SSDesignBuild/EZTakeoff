          ...
          {sections.map((section) => (
            <div key={section.id} className="section-card">
              ...
              <div className="form-grid section-input-grid">
                ...
                <label className="form-field"><span>Uprights</span><input type="number" step="1" min="0" value={section.uprights} onChange={(event) => { const nextCount = Math.max(0, Number(event.target.value) || 0); const current = Array.isArray(section.uprightOffsets) ? section.uprightOffsets.slice(0, nextCount) : []; while (current.length < nextCount) current.push(Number((((current.length + 1) * section.width) / (nextCount + 1)).toFixed(2))); updateSection(section.id, { uprights: nextCount, uprightOffsets: current }); }} /></label>
              ...
              <div className="toggle-cluster">
                ...
                <label className="toggle-field"><input type="checkbox" checked={section.pickets} onChange={(event) => updateSection(section.id, { pickets: event.target.checked, chairRail: event.target.checked ? true : section.chairRail, chairRailCount: event.target.checked ? Math.max(1, section.chairRailCount || 1) : section.chairRailCount })} /><div><span>Pickets with chair rail</span></div></label>
              </div>
              {section.chairRail && <div className="form-grid section-input-grid"><label className="form-field"><span>Horizontal chair rails</span><input type="number" min="1" step="1" value={Math.max(1, section.chairRailCount || 1)} onChange={(event) => updateSection(section.id, { chairRail: true, chairRailCount: Math.max(1, Number(event.target.value) || 1) })} /></label><label className="form-field"><span>First chair rail height (ft)</span><input type="number" min="0.5" step="0.1" value={section.chairRailHeight} onChange={(event) => updateSection(section.id, { chairRailHeight: Number(event.target.value) })} /></label></div>}
              {Math.max(0, section.uprights) > 0 && <div className="form-grid section-input-grid">{Array.from({ length: section.uprights }, (_, index) => (<label key={`upright-${index}`} className="form-field"><span>{`Upright ${index + 1} from left (ft)`}</span><input type="number" min="0" max={section.width} step="0.1" value={Array.isArray(section.uprightOffsets) && section.uprightOffsets[index] !== undefined ? section.uprightOffsets[index] : Number((((index + 1) * section.width) / (section.uprights + 1)).toFixed(2))} onChange={(event) => { const next = Array.isArray(section.uprightOffsets) ? section.uprightOffsets.slice() : []; next[index] = Math.max(0, Math.min(section.width, Number(event.target.value) || 0)); updateSection(section.id, { uprightOffsets: next }); }} /></label>))}</div>}
        ...
        {gableSections.length === 0 ? <p className="muted">No gable sections added.</p> : <div className="section-card-grid">{gableSections.map((gable) => (
          <div key={gable.id} className="section-card">
            ...
            <div className="form-grid section-input-grid">
              ...
              <label className="form-field"><span>Mounting surface</span><select value={gable.mountingSurface} onChange={(event) => updateGable(gable.id, { mountingSurface: event.target.value as GableSectionConfig['mountingSurface'] })}><option value="wood">Wood</option><option value="concrete">Masonry</option><option value="metal">Metal</option></select></label>
              <label className="form-field"><span>Mounting to sides</span><select value={gable.sideMount} onChange={(event) => updateGable(gable.id, { sideMount: event.target.value as GableSectionConfig['sideMount'] })}><option value="wood">Wood</option><option value="concrete">Masonry</option><option value="metal">Metal</option></select></label>
            </div>
          </div>
        ))}</div>}
      </section>
