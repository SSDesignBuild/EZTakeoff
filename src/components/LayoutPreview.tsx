...
function ScreenPreview({ values, renaissance }: { values: Record<string, string | number | boolean>; renaissance: boolean }) {
  ...
  {gableSections.map((gable) => { const baseY = 48 + Math.max(...gableSections.map((item)=>item.height*scale)); const gW = gable.width * scale; const baseLeft = runningGX; const baseRight = baseLeft + gW; runningGX += gW + gutter; const apexX = (baseLeft + baseRight) / 2; const apexY = baseY - gable.height * scale; const lineClass = renaissance ? 'reno-2x2-line' : 'twobytwo-line'; return <g key={gable.id}><polygon points={`${baseLeft},${baseY} ${apexX},${apexY} ${baseRight},${baseY}`} className="screen-box" /> <line x1={baseLeft} y1={baseY} x2={apexX} y2={apexY} className={lineClass} /><line x1={apexX} y1={apexY} x2={baseRight} y2={baseY} className={lineClass} /><line x1={apexX} y1={apexY} x2={apexX} y2={baseY} className={lineClass} />{gable.style === 'tied-king-post' && <line x1={baseLeft + 12} y1={baseY - 16} x2={baseRight - 12} y2={baseY - 16} className={lineClass} />}{gable.style === 'braced-king-post' && <><line x1={apexX} y1={baseY - gable.height * scale * 0.45} x2={baseLeft + gW * 0.2} y2={baseY - 14} className={lineClass} /><line x1={apexX} y1={baseY - gable.height * scale * 0.45} x2={baseRight - gW * 0.2} y2={baseY - 14} className={lineClass} /></>}{gable.style === 'queen-king-post' && <><line x1={baseLeft + gW * 0.3} y1={baseY - 8} x2={baseLeft + gW * 0.3} y2={apexY + 18} className={lineClass} /><line x1={baseRight - gW * 0.3} y1={baseY - 8} x2={baseRight - gW * 0.3} y2={apexY + 18} className={lineClass} /></>}<text x={baseLeft} y={apexY - 8} className="svg-note">{`${gable.label} · ${feetAndInches(gable.width)} × ${feetAndInches(gable.height)}`}</text></g>; })}
...
function SunroomPreview({ values }: { values: Record<string, string | number | boolean> }) {
  ...
  const y0 = 80;
  let runningX = x0;
  return <div className="visual-card"><div className="visual-header"><div><h3>Layout preview</h3><span>Elite Add-A-Room front wall sections with per-section kick panel, main opening, and transom packages.</span></div><div className="preview-toolbar"><button type="button" className="ghost-btn small-btn" onClick={() => { void exportSvgAsPdf(svgRef.current, 'Sunroom plan', 'sns-sunroom-plan.pdf'); }}>Export PDF</button></div></div><svg ref={svgRef} viewBox={`0 0 ${viewW} ${viewH}`} className="layout-svg patio-sheet-svg"><rect x={x0} y={y0} width={totalW} height={totalH} className="screen-box" rx="8" />{sections.map((section) => { const w = section.width * scale; const h = section.height * scale; const left = runningX; const top = y0 + (totalH - h); const bottom = top + h; runningX += w + gutter; const kickTop = bottom - section.kickHeight * scale; const transomNeeded = section.transomType === 'panel' || section.transomType === 'picture-window' || (section.transomType === 'auto' && section.height > 10 && section.mainSection !== 'picture-window'); const transomY = transomNeeded ? top + section.transomHeight * scale : null; return <g key={section.id}><rect x={left} y={top} width={w} height={h} className="screen-box" rx="6" /><text x={left} y={top - 10} className="svg-note">{`${section.label} · ${feetAndInches(section.width)}`}</text><line x1={left} y1={bottom} x2={left + w} y2={bottom} className="receiver-line" />{section.kickSection !== 'none' && <line x1={left} y1={kickTop} x2={left + w} y2={kickTop} className="receiver-line" />}{transomY !== null && <line x1={left} y1={transomY} x2={left + w} y2={transomY} className="trim-line" />}{Array.from({ length: section.uprights + 1 }, (_, idx) => { const x = left + ((idx + 1) * w) / (section.uprights + 2); return <line key={idx} x1={x} y1={top} x2={x} y2={bottom} className="twobytwo-line" />; })}{section.doorType !== 'none' ? <rect x={left + w * 0.25} y={bottom - (6 + 8/12) * scale} width={section.doorType === 'slider' ? 6 * scale : 3 * scale} height={(6 + 8/12) * scale} className="door-fill" rx="4" /> : <text x={left + 8} y={bottom - 12} className="svg-note">{`${section.windowCount} ${section.mainSection}`}</text>}</g>; })}{roofStyle === 'gable' ? <polygon points={`${x0},${y0} ${(x0 + totalW / 2)},${y0 - 46} ${x0 + totalW},${y0}`} className="roof-panel" /> : <line x1={x0} y1={y0} x2={x0 + totalW} y2={y0 - 28} className="roof-panel" />}<text x={x0} y={y0 + totalH + 24} className="svg-note">{`Front ${feetAndInches(frontWidth)} · Left proj ${feetAndInches(leftProjection)} · Right proj ${feetAndInches(rightProjection)}`}</text></svg><div className="legend-row wrap-legend"><span><i className="legend-swatch receiver-swatch" /> channels / caps</span><span><i className="legend-swatch twobytwo-swatch" /> uprights / H-beams</span><span><i className="legend-swatch roof-panel-swatch" /> roof / header</span></div></div>;
}
