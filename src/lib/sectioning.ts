    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((item, index) => ({
          ...
          uprightOffsets: Array.isArray(item.uprightOffsets) ? item.uprightOffsets.map((v: unknown) => Number(v)).filter((v: number) => Number.isFinite(v)) : DEFAULT_SCREEN_SECTION.uprightOffsets,
          chairRail: Boolean(item.chairRail ?? ((item.chairRailCount ?? 0) > 0)),
          chairRailCount: Math.max(0, Number(item.chairRailCount ?? ((item.chairRail ?? DEFAULT_SCREEN_SECTION.chairRail) ? 1 : 0))),
          chairRailHeight: Number(item.chairRailHeight ?? DEFAULT_SCREEN_SECTION.chairRailHeight),
          ...
          doorHeight: Number(item.doorHeight ?? DEFAULT_SCREEN_SECTION.doorHeight),
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((item, index) => ({
          ...
          doorAboveSection: (item.doorAboveSection ?? DEFAULT_SUNROOM_SECTION.doorAboveSection) as SunroomSectionConfig['doorAboveSection'],
