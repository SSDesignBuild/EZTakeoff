    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((item, index) => ({
          ...
          mainWindowCount: Number(item.mainWindowCount ?? item.windowCount ?? DEFAULT_SUNROOM_SECTION.mainWindowCount),
          kickWindowCount: Number(item.kickWindowCount ?? DEFAULT_SUNROOM_SECTION.kickWindowCount),
          transomWindowCount: Number(item.transomWindowCount ?? DEFAULT_SUNROOM_SECTION.transomWindowCount),
